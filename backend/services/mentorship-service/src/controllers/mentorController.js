...
exports.getMentorReviews = async (req, res) => {
  try {
    const payload = await mentorService.getMentorReviews(req.params.id);
    res.status(200).json({ data: payload });
  } catch (error) {
    console.error('Error fetching mentor reviews:', error);
    res.status(error.status || 500).json({ message: 'Error fetching mentor reviews', error: error.message });
  }
};
