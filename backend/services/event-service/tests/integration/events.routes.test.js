...
describe('GET /api/events', () => {
  it('200 — returns empty list when no events', async () => {
    ...
    expect(res.status).toBe(200);
    const items = res.body.data || res.body.events || res.body;
...
describe('POST /api/events', () => {
  it('201 — mentor creates an event', async () => {
    ...
    expect(res.body.title).toBe('Women in Tech Summit');
  });
...
describe('GET /api/events/:id', () => {
  it('200 — returns event details', async () => {
    ...
    const res = await request(app).get(`/api/events/${createRes.body._id}`);
    expect(res.status).toBe(200);
    expect(res.body.title).toBe('Women in Tech Summit');
  });
...
describe('PATCH /api/events/:id', () => {
  it('200 — creator can update their event', async () => {
    ...
    const updateRes = await request(app)
      .patch(`/api/events/${createRes.body._id}`)
      .set('Authorization', `Bearer ${token}`)
      .send({ title: 'Updated Summit Title' });

    expect(updateRes.status).toBe(200);
    expect(updateRes.body.title).toBe('Updated Summit Title');
  });
});

// ── DELETE /api/events/:id ─────────────────────────────────────────────────────
describe('DELETE /api/events/:id', () => {
  it('200 — creator can delete their event', async () => {
    ...
    const deleteRes = await request(app)
      .delete(`/api/events/${createRes.body._id}`)
      .set('Authorization', `Bearer ${token}`);
...
describe('POST /api/events/:id/register', () => {
  it('200 — mentee can register for an event', async () => {
    ...
    const regRes = await request(app)
      .post(`/api/events/${createRes.body._id}/register`)
  ...
  it('401 — unauthenticated cannot register', async () => {
    ...
    const res = await request(app).post(`/api/events/${createRes.body._id}/register`);
    expect(res.status).toBe(401);
  });
});
