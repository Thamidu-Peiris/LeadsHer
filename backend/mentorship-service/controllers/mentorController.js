...
exports.createOrUpdateMentorProfile = async (req, res) => {
  try {
    const { expertise, yearsOfExperience, industries, mentoringAreas, availability, bio, achievements } = req.body;
    ...
    if (mentorProfile) {
      ...
      mentorProfile = await MentorProfile.create({
        user: req.user._id, expertise, yearsOfExperience, industries, mentoringAreas,
    ...
    res.status(400).json({ message: 'Error creating/updating mentor profile', error: error.message });
  }
};

exports.getAllMentors = async (req, res) => {
  try {
    const { expertise, industry, minExperience, maxExperience, availableOnly, page = 1, limit = 10, sort = '-rating' } = req.query;
    const filter = {};
...
exports.getMentorById = async (req, res) => {
  try {
    const mentorProfile = await MentorProfile.findById(req.params.id).populate('user', 'name email avatar bio');
...
exports.getMentorByUserId = async (req, res) => {
  try {
    const mentorProfile = await MentorProfile.findOne({ user: req.params.userId }).populate('user', 'name email avatar bio');
    if (!mentorProfile) return res.status(404).json({ message: 'Mentor profile not found' });
    res.status(200).json({ data: mentorProfile });
  } catch (error) {
    console.error('Error fetching mentor profile:', error);
...
exports.getMyMentorProfile = async (req, res) => {
  try {
    const mentorProfile = await MentorProfile.findOne({ user: req.user._id }).populate('user', 'name email avatar bio');
    if (!mentorProfile) return res.status(404).json({ message: 'Mentor profile not found' });
    res.status(200).json({ data: mentorProfile });
  } catch (error) {
    console.error('Error fetching mentor profile:', error);
...
exports.getMentorStats = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error fetching mentor statistics', error: error.message });
  }
};
