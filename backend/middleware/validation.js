...
exports.validateMentorProfile = (req, res, next) => {
  ...
  if (expertise && (!Array.isArray(expertise) || expertise.length === 0)) {
    errors.push('Expertise must be a non-empty array');
  }
  ...
  // Check mentoring areas
  if (mentoringAreas && (!Array.isArray(mentoringAreas) || mentoringAreas.length === 0)) {
    errors.push('Mentoring areas must be a non-empty array');
  }
  ...
  // Check availability
  if (availability) {
    if (availability.maxMentees !== undefined) {
      const max = parseInt(availability.maxMentees);
      if (isNaN(max) || max < 1 || max > 10) {
        errors.push('Max mentees must be between 1 and 10');
      }
    }

    if (availability.preferredTime && !Array.isArray(availability.preferredTime)) {
      errors.push('Preferred time must be an array');
    }
...
exports.validateMentorshipRequest = (req, res, next) => {
  ...
  if (!mentorId || typeof mentorId !== 'string') {
    errors.push('Valid mentor ID is required');
  }
  ...
  if (goals && goals.some(goal => typeof goal !== 'string' || goal.trim() === '')) {
    errors.push('All goals must be non-empty strings');
  }
  ...
  if (!preferredStartDate) {
    ...
    if (isNaN(date.getTime())) {
      ...
      errors.push('Preferred start date must be in the future');
    }
  }
