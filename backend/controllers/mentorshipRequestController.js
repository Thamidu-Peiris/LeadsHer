  ...
  try {
    ...
    if (!mentorId || !goals || !preferredStartDate || !message) {
      return res.status(400).json({
        message: 'Please provide mentorId, goals, preferredStartDate, and message',
      });
    }
    ...
    // Check if mentor is verified and available
    if (!mentorProfile.isVerified) {
      return res.status(400).json({
        message: 'This mentor is not verified to accept requests',
      });
    }

    if (!mentorProfile.canAcceptMentees) {
      return res.status(400).json({
        message: 'This mentor is not currently accepting new mentees',
      });
    }
    ...
    if (activeMentorship) {
      return res.status(400).json({
        message: 'You already have an active mentorship. Complete it before requesting a new one.',
      });
    }
    ...
    // Validate preferred start date is in the future
...
// @desc    Get all mentorship requests (for current user)
...
exports.getMentorshipRequests = async (req, res) => {
  try {
    ...
    // Build filter based on user role and type
    const filter = {};
    
    if (type === 'received' && req.user.role === 'Mentor') {
...
exports.getMentorshipRequestById = async (req, res) => {
  try {
    ...
    if (
      ...
      request.mentee._id.toString() !== req.user._id.toString()
    ) {
    ...
    res.status(500).json({
      message: 'Error fetching mentorship request',
      error: error.message,
    });
  }
};
...
exports.acceptMentorshipRequest = async (req, res) => {
  try {
    ...
    // Check if mentor profile still allows new mentees
    const mentorProfile = await MentorProfile.findOne({ user: req.user._id });
    if (!mentorProfile.canAcceptMentees) {
      return res.status(400).json({
        message: 'You cannot accept new mentees at this time',
      });
    }
    ...
    if (activeMentorship) {
      return res.status(400).json({
        message: 'This mentee already has an active mentorship',
      });
    }
    ...
    res.status(500).json({
      message: 'Error accepting mentorship request',
      error: error.message,
    });
  }
};
...
exports.rejectMentorshipRequest = async (req, res) => {
  try {
    ...
    if (request.mentor.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        message: 'Only the mentor can reject this request',
      });
    }
    ...
    res.status(500).json({
      message: 'Error rejecting mentorship request',
      error: error.message,
    });
  }
};

// @desc    Cancel mentorship request (by mentee)
// @route   PUT /api/mentorship/requests/:id/cancel
...
exports.cancelMentorshipRequest = async (req, res) => {
  try {
    ...
    if (request.mentee.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        message: 'Only the mentee can cancel this request',
      });
    }
    ...
    res.status(500).json({
      message: 'Error cancelling mentorship request',
      error: error.message,
    });
  }
};
